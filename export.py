    ...
    with ZipFile(options.zipfile, "r") as zip:
        for name in zip.namelist():
            ...
            if path.stem.isdigit():
                datadir = options.datadir_valid if (count % 10 == 0) else options.datadir_train
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "zipfile", type=pathlib.Path, help="The zip-file to export"
    )
    parser.add_argument(
        "--datadir-train", type=pathlib.Path, required=True, help="Destination training data directory"
    )
    parser.add_argument(
        "--datadir-valid", type=pathlib.Path, required=True, help="Destination validation data directory"
    )
